...
from load_data import load_bike_train, inverse_transform, load_bike_test
from EnvConfounderIRM import EnvAware

path = '/data/user18100643/dataset/filter_bike/'
...
batchnum= 1500
learning_rate = 1e-3
div_en_n = 2  # the number of environments
current_time = dt.now().strftime("%Y%m%d-%H%M%S")
reg_weight = 0
irm_weight_init = 10.0
contrs_weight_init = 10.0
...
# load data
f_name = path + city + 'l'+str(seq_len) +'s'+str(span)+ '.npz'
data = np.load(f_name)
test_set = load_bike_test(data, inorout)
...
test_mape_err = tf.keras.metrics.Mean(name='test_mape')
...
def train_step(train_env_set,div_en_n,rec_flg,irm_weight,contrs_weight):
    ...
    with tf.GradientTape(persistent=True) as g:
        ...
        # regulazation
        # l2_reg = tf.reduce_mean([tf.nn.l2_loss(v) for v in MODEL.trainable_variables[0:6]])
        # y_loss = MSE_loss+ reg_weight * l2_reg

        loss_contrastive = MODEL.loss_c(env_feature_set, env_label_set)

        loss4irm = loss_y+irm_weight*loss_irm +reg_weight*tf.reduce_mean([tf.nn.l2_loss(v) for v in MODEL.get_env_irm_v()])
        # if irm_weight>1.:   loss4irm /= irm_weight

        loss4contrs = loss_y+contrs_weight*loss_contrastive + reg_weight*tf.reduce_mean([tf.nn.l2_loss(v) for v in MODEL.get_env_rel_v()])
        # if contrs_weight>1.: loss4contrs/=contrs_weight

        loss4pred = loss_y+contrs_weight*loss_contrastive+irm_weight*loss_irm +reg_weight*tf.reduce_mean([tf.nn.l2_loss(v) for v in MODEL.get_out_v()])
    ...
    return tf.concat(outputs_env,axis=0), tf.concat(y_true_env,axis=0), loss_y
...
def test(s,rec_flg):
    ...
    rmse1 = np.sqrt(test_mse_err.result())
    print('RMSE for test set 1:', rmse1)
    print('MSE for test set 1:', test_mse_err.result())
    with result_summary_writer.as_default():
        tf.summary.scalar(name='test_rmse', data=rmse1, step=s)
        tf.summary.scalar(name='test_mse', data=test_mse_err.result(), step=s)
        tf.summary.scalar(name='test_mae', data=test_mae_err.result(), step=s)
        tf.summary.scalar(name='test_mape', data=test_mape_err.result(), step=s)

    # record_res(test_pred, 1)
    test_mse_err.reset_state()
    test_mae_err.reset_state()
    test_mape_err.reset_state()
...
record_flg=0.
generater = load_bike_train(data,div_en_n,inorout,batchsize)
for s in range(steps):
    ...
    for batch in range(batchnum):
        if s==steps-1 and batch == batchnum-1: record_flg=1.
        with tf.GradientTape() as g:
            tr_set=next(generater)
    ...
    with result_summary_writer.as_default():
        ...
        tf.summary.scalar(name='train_real_mse', data=real_mse, step=s)
